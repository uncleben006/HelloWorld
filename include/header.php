				...
				<td rowspan="2" class="top_notify_td01"><!--通知欄-->
					<?php
						if(isset($_SESSION['account'])){
							...
							$selectRemindHost = "SELECT * FROM `remind` WHERE `host` = '".$account."' AND `decide` = '3'";
							...
							$hostNum = mysql_num_rows($selectRemindHost);

							if($remindNum>0||$hostNum>0){
								//如果有account的提醒，就做這個判斷
								if($remindNum>0){
								...
								//如果有host的提醒(也就是說你是房主，有人加入你時)，就做這個判斷
								if($hostNum>0){
									while($remindHost = mysql_fetch_assoc($selectRemindHost)){
										$click = $remindHost['click'];
										if($click==0){
											session_set_cookie_params(99999);
											$_SESSION['click'] = 0;
										}
									...
									<div class="notify_outfram">
										<div class="notify_fram" id="remindDelete">
											<?php
												...
												/*原選取sql原提醒迴圈*/
												/*
												$selectRemindAccount = "SELECT * FROM `remind` WHERE `account` = '".$account."' ORDER BY `num` DESC";
												mysql_query("SET NAMES'UTF8'");
												mysql_query("SET CHARACTER SET UTF8");
												mysql_query("SET CHARACTER_SET_RESULTS='UTF8'");
												$selectRemindAccount = mysql_query($selectRemindAccount);

												$selectRemindHost = "SELECT * FROM `remind` WHERE `host` = '".$account."' ORDER BY `num` DESC";
												mysql_query("SET NAMES'UTF8'");
												mysql_query("SET CHARACTER SET UTF8");
												mysql_query("SET CHARACTER_SET_RESULTS='UTF8'");
												$selectRemindHost = mysql_query($selectRemindHost);
													...
													if($remindAccount['decide']==1){
														?>
															<div class="notify_div01">
													            ...
													            <div class="notify_div_p">
													                <p>您於剛剛被<a href="http://www.jomorparty.com/system/group/userData.php?account=<?php echo $remindAccount['host'];?>"><font color="blue"><?php echo $hostName; ?></font></a>踢出了房間，房名為<font color="red"><?php echo $remindAccount['room']; ?></font></p>
													            </div>
															</div>
													  	<?php  
													}
													if($remindAccount['decide']==2){
														?>
															<div class="notify_div01">
													            ...
													            <div class="notify_div_p">
													                <p><a href="http://www.jomorparty.com/system/group/userData.php?account=<?php echo $remindAccount['host'];?>"><font color="blue"><?php echo $hostName; ?></font></a>因為個人因素刪除了房間，房名為<font color="red"><?php echo $remindAccount['room']; ?></font></p>
													            </div>
															</div>
													  	<?php  
													}
													if($remindAccount['decide']==3){
														if($hostPri==2){
															?>
															<div class="notify_div01">
													            <div class="notify_div_img">
													            ...
													            <div class="notify_div_p">
													                <p>您剛剛加入了<a href="http://www.jomorparty.com/system/group/userData.php?account=<?php echo $remindAccount['host'];?>"><font color="blue"><?php echo $hostName; ?></font></a>的房間，房名為<font color="red"><?php echo $remindAccount['room']; ?></font></p>
													            </div>
															</div>
															<?php
														}
