	...
	<body id="body0">
		<?php 
			...
			if(isset($_POST['submit'])){
				...
				$selectUserName = "SELECT * FROM `user` WHERE `name`='".$name."'";
				$selectUserName = mysql_query($selectUserName);
				$userName = mysql_fetch_assoc($selectUserName);
				...
				else if(isset($userAccount["account"])){//判斷帳號重複
				...
				else if(isset($userEmail["email"])){//判斷帳號重複
					$url = "signup.php?wrong=9";
					echo "<script type='text/javascript'>";
					echo "window.location.href='$url'";
					echo "</script>";
				}
									...
									<td class="register_td2">
										<?php
											if($wrong==0){
												...
												?>
												<div><font color="#E86363"><?php echo "信箱已經註冊，請選登入帳號" ?></font></div>
												<?php
											}
											else{
												?>
